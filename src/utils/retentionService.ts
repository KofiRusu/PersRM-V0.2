...
export class RetentionService {
  ...
  constructor() {
    ...
    this.sessionId =
      localStorage.getItem("reasoningAssistantCurrentSession") || null;
  ...
  filterEvents(options: {
    ...
    if (options.startDate) {
      filteredEvents = filteredEvents.filter(
        (event) => new Date(event.timestamp) >= options.startDate!,
      );
    }

    if (options.endDate) {
      filteredEvents = filteredEvents.filter(
        (event) => new Date(event.timestamp) <= options.endDate!,
      );
    }
    ...
    if (options.sessionId) {
      filteredEvents = filteredEvents.filter(
        (event) => event.sessionId === options.sessionId,
      );
    }
