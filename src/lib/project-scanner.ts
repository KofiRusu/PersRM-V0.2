...
export class ProjectScanner {
  ...
  private async detectProjectType(projectPath: string): Promise<string> {
    try {
      ...
      const hasElectronInDeps = hasPackageJson && 
  ...
  private async identifyFramework(projectPath: string): Promise<string> {
    try {
      ...
      const maxIndicator = Object.entries(indicators).reduce(
        (max, [framework, count]) => count > max.count ? { framework, count } : max,
   ...
   */
  private async findComponents(projectPath: string, framework: string): Promise<ComponentInfo[]> {
    try {
        ...
        componentFiles.slice(0, 100).map(async file => {
          try {
            ...
            if (framework === 'react') {
              ...
              componentName = funcMatch?.[1] || constMatch?.[1] || classMatch?.[1] || componentName;
            }
  ...
  private extractComponentProps(content: string, framework: string): string[] {
    ...
    try {
      if (framework === 'react') {
        ...
        const propTypesMatch = content.match(/([A-Za-z0-9_]+)\.propTypes\s*=\s*{([^}]+)}/);
        if (propTypesMatch) {
