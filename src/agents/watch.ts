    ...
    const agentWrapper: PersRMAgentWrapper = {
      ...
      analyzeDirectory: async (directoryPath: string) => {
        try {
          ...
          console.error(
            chalk.red(`Directory analysis error: ${error.message}`),
          );
          return [];
        }
      },
...
async function watchComponents(watchPath: string, options: WatchOptions) {
  ...
  console.log(
    chalk.white(
      `Mode: ${options.mode.toUpperCase() || process.env.PERSRM_MODE?.toUpperCase() || "MOCK"}`,
    ),
  );
  console.log(chalk.gray("---------------------------"));
  ...
  try {
    ...
    console.error(
      chalk.red(
        `Could not create watch directory: ${error?.message || "Unknown error"}`,
      ),
    );
  }
  ...
  watcher.on("change", async (filePath) => {
    ...
    if (now - lastOptimizationTime < DEBOUNCE_TIME) {
      console.log(
        chalk.yellow(`Change detected in ${relativePath} (debounced)`),
      );
      return;
    }
    ...
    if (options.autoOptimize) {
      ...
      try {
        ...
        if (options.benchmark) {
          try {
            const { BenchmarkTracker } = await import(
              "../lib/benchmarks/BenchmarkTracker"
            );
        ...
        if (optimizationResult) {
          ...
          console.log(
            chalk.white(
              `Score: ${analysis.summary.overallScore}/${analysis.summary.maxScore}`,
            ),
          );
        ...
        console.error(
          chalk.red(
            `Error during optimization: ${error?.message || String(error) || "Unknown error"}`,
          ),
        );
        console.log(chalk.gray("---------------------------"));
      }
      ...
      console.log(
        chalk.white(
          "Auto-optimization is disabled. Use --auto-optimize to enable.",
        ),
      );
      console.log(chalk.gray("---------------------------"));
    }
  ...
  watcher.on("error", (error: any) => {
    console.error(
      chalk.red(`Watcher error: ${error?.message || String(error)}`),
    );
  });
  ...
  setTimeout(async () => {
    ...
    if (!(await fs.pathExists(testFile))) {
      console.log(
        chalk.gray("Creating a test component to demonstrate the watcher..."),
      );
      await fs.writeFile(
        testFile,
        `
...
`,
      ...
      console.log(
        chalk.blue(
          `Created test file: ${path.relative(workspacePath, testFile)}`,
        ),
      );
    }
  }, 1000);
...
program
  ...
  .action((watchPath, options) => {
    watchComponents(watchPath, options).catch((error: any) => {
      console.error(
        chalk.red(
          `Failed to start watcher: ${error?.message || String(error)}`,
        ),
      );
      process.exit(1);
    });
