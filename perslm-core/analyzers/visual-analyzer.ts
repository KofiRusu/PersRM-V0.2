...
export class VisualAnalyzer {
  ...
  public async compareWithBaseline(
    ...
    for (const state of componentConfig.states) {
      ...
      if (!fs.existsSync(baselinePath)) {
        if (this.config.verbose) {
          console.log(
            `No baseline found for ${componentName} in state: ${state}`,
          );
        }
        continue;
      }
   ...
   */
  public async analyzeColorConsistency(
    componentName: string,
  ...
  private findColorInconsistencies(
    ...
    if (colors.length > 8) {
      inconsistencies.push({
        ...
        recommendation:
          "Reduce the number of unique colors for better visual consistency. Consider using a design token system.",
      });
    }
   ...
   */
  public async checkContrastRatio(
    foreground: string,
    background: string,
  ): Promise<{
    passes: boolean;
  ...
  public async generateReport(
    ...
    for (const issue of accessibilityResult.contrastIssues) {
      visualIssues.push({
        ...
        impact: "High",
        recommendation:
          "Increase contrast between foreground and background colors to meet WCAG AA standards",
      });
    }
    ...
    const recommendations = [
      ...new Set(visualIssues.map((issue) => issue.recommendation)),
    ];
    ...
    // Collect diff images
    const diffImages = comparisonResult.differences.map(
