...
export class AnimationPerformanceTracker {
  ...
  public registerComponent(
    ...
    if (this.config.verbose) {
      console.log(
        `Registered ${animations.length} animations for component ${componentId}`,
      );
    }
  }
  ...
  private simulateFrameData(
    ...
    const idealFrameTime = 1000 / targetFps;
    const dropCount = deltas.filter(
      (delta) => delta > idealFrameTime * 1.5,
    ).length;
  ...
  public async analyzeComponent(
    ...
    for (const animation of componentAnimations) {
      try {
        ...
        const animationIssues = this.detectPerformanceIssues(
          componentId,
          animation.name,
          frameData,
        );
        ...
        console.error(
          `Error tracking animation "${animation.name}" for component ${componentId}:`,
          error,
        );
      }
    }
    ...
    if (componentAnimations.length === 0) {
      try {
        ...
        console.error(
          `Error tracking general performance for component ${componentId}:`,
          error,
        );
      }
    }
  ...
  private detectPerformanceIssues(
    ...
    if (frameData.averageFps < targetFps * 0.9) {
      issues.push({
        ...
        severity:
          frameData.averageFps < targetFps * 0.75
            ? SeverityLevel.ERROR
            : SeverityLevel.WARNING,
        impact: frameData.averageFps < targetFps * 0.75 ? "High" : "Medium",
        suggestion:
          "Optimize animation performance by reducing complexity or using CSS transitions",
    ...
    if (
      frameData.dropCount > 5 ||
      frameData.dropCount / frameData.frameDeltas.length > 0.1
    ) {
      issues.push({
        id: uuidv4(),
        component: componentId,
        ...
        impact: "Medium",
        suggestion:
          "Reduce animation complexity or optimize rendering performance",
        frameRate: frameData.averageFps,
        expectedFrameRate: targetFps,
      });
    }
    ...
    if (frameData.minFps < targetFps * 0.5) {
      issues.push({
        ...
        impact: "High",
        suggestion:
          "Look for blocking operations during animation that cause severe stuttering",
  ...
  private calculatePerformanceScore(
    ...
    const targetFps = this.config.targetFrameRate;
    const averageFpsRatio =
      animationResults.reduce(
  ...
  private parseFrameData(rawData: string): FrameRateData {
    try {
      ...
      const dropCount = frameDeltas.filter(
        (delta) => delta > idealFrameTime * 1.5,
      ).length;
