...
export class DesignTokenAnalyzer {
    ...
    try {
      if (fs.existsSync(this.config.designSystemPath)) {
        ...
        if (this.config.verbose) {
          console.log(
            `Loaded design system from ${this.config.designSystemPath}`,
          );
        }
      } else {
        ...
        if (this.config.verbose) {
          console.log(
            `Design system file not found at ${this.config.designSystemPath}, using defaults`,
          );
        }
      }
    } catch (error) {
  ...
  private loadTokenDefinitions(): void {
    try {
      ...
      if (fs.existsSync(tokenDir)) {
        ...
        if (this.config.verbose) {
          console.log(
            `Loaded ${this.tokenDefinitions.size} token definitions from ${tokenFiles.length} files`,
          );
        }
  ...
  private extractTokensFromDesignSystem(): void {
    ...
    // Extract typography
    Object.entries(this.designSystem.typography || {}).forEach(
      ([name, value]) => {
    ...
    if (this.config.verbose) {
      console.log(
        `Extracted ${this.tokenDefinitions.size} token definitions from design system`,
      );
    }
  }
   ...
   */
  public async extractTokensFromComponent(
    ...
    try {
      ...
      const componentTokens = this.extractTokensFromContent(
        content,
        componentPath,
      );
      ...
      if (this.config.includeCssFiles) {
        const cssTokens = await this.extractTokensFromCssFiles(
          componentPath,
          componentName,
        );
      ...
      this.extractedTokens.set(
        componentName,
        new Set(tokensFound.map((t) => t.name)),
      );
   ...
   */
  private extractTokensFromContent(
    content: string,
    filePath: string,
  ): DesignToken[] {
    const tokensFound: DesignToken[] = [];
      ...
      while ((match = styleRegex.exec(content)) !== null) {
        ...
        for (const [prop, value] of styleProps) {
          ...
          if (/color|background|border-color|fill|stroke/.test(prop)) {
            ...
            /margin|padding|width|height|top|left|right|bottom|gap/.test(prop)
          ) {
            tokenType = "spacing";
          } else if (
            /font|text|line-height|letter-spacing|word-spacing/.test(prop)
          ) {
            tokenType = "typography";
  ...
  private extractImportedTokens(content: string): DesignToken[] {
    ...
    const importRegex =
      /import\s+(\{[\s\S]*?\}|\*\s+as\s+\w+)\s+from\s+['"](.+?(?:tokens|theme|design|styles)[^'"]*)['"]/g;
    let importMatch;
    ...
    while ((importMatch = importRegex.exec(content)) !== null) {
      ...
      if (tokenNamesMatch) {
        const tokenNames = tokenNamesMatch[1]
          .split(",")
          .map((name) => name.trim());
        ...
        for (const tokenName of tokenNames) {
          ...
          if (usageRegex.test(content)) {
            ...
            const matchingDefinitions = Array.from(
              this.tokenDefinitions.values(),
   ...
   */
  private async extractTokensFromCssFiles(
    componentPath: string,
    componentName: string,
  ): Promise<DesignToken[]> {
    const tokensFound: DesignToken[] = [];
    ...
    const cssFiles = glob.sync(
      path.join(dir, `${baseFileName}.{css,scss,less}`),
    );
          ...
          if (rule.type === "rule" && rule.declarations) {
            for (const declaration of rule.declarations) {
              if (
                ...
                // Determine token type based on property name
                if (
                  /color|background|border-color|fill|stroke/.test(
                    declaration.property,
                  )
                ) {
                  tokenType = "color";
                } else if (
                  /margin|padding|width|height|top|left|right|bottom|gap/.test(
                    declaration.property,
                  )
                ) {
                  tokenType = "spacing";
                } else if (
                  /font|text|line-height|letter-spacing|word-spacing/.test(
                    declaration.property,
                  )
                ) {
                  tokenType = "typography";
                } else if (
                  /shadow|box-shadow|text-shadow/.test(declaration.property)
                ) {
                  tokenType = "shadow";
                } else if (
                  /border|border-radius|border-width/.test(declaration.property)
                ) {
                  tokenType = "border";
  ...
  private validateTokens(
    ...
    for (const token of tokens) {
      ...
      if (token.type === "color") {
        ...
        const definedColors = Object.values(this.designSystem.colors || {}).map(
        ...
        if (!definedColors.includes(colorValue)) {
          ...
          if (closestColor) {
            ...
            issues.push({
              ...
              impact: "Low",
              suggestion:
                "Consider adding this color to the design system if it's intended to be reused",
            });
          }
        }
      }
      ...
      if (token.type === "spacing") {
        ...
        const definedSpacing = Object.values(
        ...
        if (!definedSpacing.includes(spacingValue)) {
          issues.push({
            ...
            impact: "Low",
            suggestion:
              "Use design system spacing tokens for consistent layout",
          });
        }
      }
    }
  ...
  public async analyzeDirectory(
    ...
    try {
      ...
      const files = glob.sync(
        path.join(directoryPath, "**/*.{jsx,tsx,js,ts}"),
        {
      ...
      if (this.config.verbose) {
        console.log(
          `Found ${files.length} files to analyze in ${directoryPath}`,
        );
      }
