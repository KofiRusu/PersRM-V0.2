...
} from "../lib/persrm/agent-switcher";
...
program
  .option(
    "-m, --mode <mode>",
    'Agent mode: "mock" or "prod"',
    process.env.PERSRM_MODE || "mock",
  )
...
program
  ...
  .action(async (targetPath, options, command) => {
    ...
    try {
      ...
      const agentMode =
        parentOptions.mode === "prod" ? AgentMode.PROD : AgentMode.MOCK;
      ...
      if (isDirectory && agent.analyzeDirectory) {
        ...
        console.log(
          chalk.green(
            `Analysis complete. Analyzed ${results.length} components.`,
          ),
        );
      } else {
        ...
        if (result && result.summary) {
          ...
          console.log(
            chalk.white(
              `Score: ${result.summary.overallScore}/${result.summary.maxScore}`,
            ),
          );
    ...
    try {
      ...
      if (result && result.suggestions) {
        ...
        if (options.apply) {
          ...
          console.log(
            chalk.yellow(`Run with --apply to apply suggested optimizations.`),
          );
        }
      } else {
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (options.diff) {
        ...
        console.log(
          chalk.white(`Current: ${new Date(current.timestamp).toISOString()}`),
        );
        console.log(
          chalk.white(
            `Score: ${current.summary.overallScore}/${current.summary.maxScore}`,
          ),
        );
        ...
        if (comparison.previous) {
          ...
          console.log(
            chalk.white(
              `Previous: ${new Date(previous.timestamp).toISOString()}`,
            ),
          );
          console.log(
            chalk.white(
              `Score: ${previous.summary.overallScore}/${previous.summary.maxScore}`,
            ),
          );
          ...
          if (delta) {
            ...
            console.log(
              color(
                `Score Change: ${scoreChange >= 0 ? "+" : ""}${scoreChange.toFixed(1)} (${delta.percentChange.toFixed(1)}%)`,
              ),
            );
            ...
            if (delta.phaseDeltas) {
              Object.entries(delta.phaseDeltas).forEach(([phase, change]) => {
                ...
                console.log(
                  phaseColor(
                    `${phase}: ${change >= 0 ? "+" : ""}${change.toFixed(1)}`,
                  ),
                );
              });
            }
            ...
            console.log(
              chalk.green(`Resolved: ${delta.resolvedIssues.length}`),
            );
            console.log(chalk.red(`New: ${delta.newIssues.length}`));
            console.log(
              chalk.white(`Unchanged: ${delta.unchangedIssues.length}`),
            );
          }
        } else {
          console.log(
            chalk.yellow("No previous benchmark found for comparison"),
          );
        }
        ...
        console.log("");
        console.log(
          chalk.white(`Latest score: ${scores[scores.length - 1].toFixed(1)}`),
        );
        console.log(
          chalk.white(
        ...
        if (scores.length > 1) {
          ...
          console.log(
            trendColor(
              `Overall trend: ${overallChange >= 0 ? "+" : ""}${overallChange.toFixed(1)} points`,
            ),
          );
        }
      } else {
        ...
        history.forEach((entry, index) => {
          ...
          console.log(
            chalk.white(
              `Score: ${entry.summary.overallScore}/${entry.summary.maxScore}`,
            ),
          );
          ...
          if (entry.summary.componentName) {
            console.log(
              chalk.white(`Component: ${entry.summary.componentName}`),
            );
          }
          ...
          if (entry.delta) {
            ...
            console.log(
              color(
                `Score Change: ${scoreChange >= 0 ? "+" : ""}${scoreChange.toFixed(1)} points`,
              ),
            );
          }
        ...
        console.log("");
        console.log(
          chalk.gray(`Showing ${history.length} of ${limit} results`),
        );
      }
    } catch (error) {
      console.error(
        chalk.red(
          `Error retrieving benchmarks: ${error instanceof Error ? error.message : "Unknown error"}`,
        ),
      );
      process.exit(1);
    }
  ...
  .action(async (resultPath, options, command) => {
    ...
    try {
      ...
      if (agent.generateReportFromResult) {
        const reportResult = await agent.generateReportFromResult(
          absolutePath,
          {
        ...
        console.log(
          chalk.red("Report generation not supported by the current agent."),
        );
      }
    } catch (error) {
      console.error(
...
program
  ...
  .action(async (options, command) => {
    ...
    try {
      ...
      const { generateSampleUserFlows, simulateUserFlow, createUserFlow } =
      ...
      if (options.simulate) {
        ...
        if (typeof options.simulate === "string") {
          const namedFlow = flows.find(
            (f) => f.name.toLowerCase() === options.simulate.toLowerCase(),
          );
          if (namedFlow) {
            ...
            console.log(
              chalk.yellow(
                `Flow "${options.simulate}" not found. Using "${targetFlow.name}" instead.`,
              ),
            );
          }
        }
        ...
        if (options.component) {
          ...
          if (analysisResult && analysisResult.summary) {
            ...
            console.log(
              chalk.red(`Failed to analyze component ${options.component}`),
            );
            process.exit(1);
          }
        } else {
          ...
          if (Object.keys(componentMap).length === 0) {
            console.log(
              chalk.yellow(
                "No analyzed components found. Run analyze command first or specify a component.",
              ),
            );
            process.exit(1);
          }
        ...
        const avgSuccessRate =
          simulationResults.reduce((sum, r) => sum + r.successRate, 0) /
          simulationResults.length;
        const avgCompletionRate =
          simulationResults.reduce((sum, r) => sum + r.completionRate, 0) /
          simulationResults.length;
        const avgConfusionRate =
          simulationResults.reduce((sum, r) => sum + r.confusionRate, 0) /
          simulationResults.length;
        const avgOverallScore =
          simulationResults.reduce((sum, r) => sum + r.overallScore, 0) /
          simulationResults.length;
        ...
        console.log(chalk.gray("---------------------"));
        console.log(
          chalk.white(`Success Rate: ${(avgSuccessRate * 100).toFixed(1)}%`),
        );
        console.log(
          chalk.white(
            `Completion Rate: ${(avgCompletionRate * 100).toFixed(1)}%`,
          ),
        );
        console.log(
          chalk.white(
            `Confusion Rate: ${(avgConfusionRate * 100).toFixed(1)}%`,
          ),
        );
        console.log(
          chalk.white(`Overall Score: ${avgOverallScore.toFixed(1)}/100`),
        );
      ...
      if (options.learn && !options.simulate) {
        ...
        if (results && results.length > 0) {
          ...
          console.log(
            chalk.green(`Learning data updated with latest analysis.`),
          );
        } else {
          console.log(
            chalk.yellow(
              "No analyzed components found. Run analyze command first.",
            ),
          );
        }
      }
      ...
      if (options.stuck) {
        ...
        if (stagnantComponents.length === 0) {
          ...
          stagnantComponents.forEach((component) => {
            ...
            console.log(
              chalk.gray(
                `  Stagnant since: ${new Date(component.stagnantSince || "").toLocaleDateString()}`,
              ),
            );
            console.log(
              chalk.gray(
                `  Optimization attempts: ${component.optimizationAttempts}`,
              ),
            );
            ...
            if (component.suggestions.length > 0) {
              ...
              component.suggestions
                ...
                .forEach((suggestion) => {
                  ...
                  console.log(
                    priorityColor(
                      `    ${suggestion.description} (${suggestion.priority.toUpperCase()})`,
                    ),
                  );
                });
            }
        ...
        console.log(
          chalk.white(`Total components tracked: ${allComponents.length}`),
        );
        console.log(
          chalk.white(`Stagnant components: ${stagnantComponents.length}`),
        );

        if (allComponents.length > 0) {
          const avgImprovementRate =
            allComponents.reduce((sum, c) => sum + c.improvementRate, 0) /
            allComponents.length;
          console.log(
            chalk.white(
              `Average improvement rate: ${(avgImprovementRate * 100).toFixed(1)}%`,
            ),
          );
        }
      }
      ...
      console.error(
        chalk.red(
          `Feedback command failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        ),
      );
      process.exit(1);
    }
...
program
  ...
  .option(
    "--min-severity <level>",
    "Minimum severity level for Slack alerts (info, warning, error, critical)",
  )
  .option(
    "--push <type>",
    "Push data to configured integrations (analysis, benchmark, learning, regression, insights)",
  )
  .option("--test", "Test the configured integrations")
  .option("--summary", "Show only summary information when pushing data")
  .option(
    "--failures",
    "Only push failed components/analyses (with critical/error issues)",
  )
  .option(
    "--last <n>",
    "Sync the last N analysis results instead of just the latest",
    "1",
  )
  .option(
    "--copy-links",
    "Generate and display copyable links to Notion/Slack",
    false,
  )
  .option(
    "--regression-threshold <num>",
    "Score delta threshold to trigger regression alerts",
    "-5",
  )
  .option(
    "--organize-by <field>",
    "Organization field for Notion data (project, component, phase, date)",
    "project",
  )
  .option(
    "--format <style>",
    "Formatting style for Slack messages (compact, detailed)",
    "detailed",
  )
  .option(
    "--group-by <field>",
    "Group entries in Notion (project, component, score, date)",
    "project",
  )
  .option(
    "--enable-ai-insights",
    "Enable AI-generated insights for regressions",
  )
  .option(
    "--force-insights",
    "Generate and push AI insights without waiting for regressions",
  )
  .action(async (options) => {
    ...
    try {
      ...
      if (
        options.notionToken ||
        options.notionDb ||
        options.enableNotion ||
        options.disableNotion ||
        options.groupBy ||
        options.organizeBy
      ) {
        const notionConfig = {};
        ...
        if (options.notionDb) {
          ...
          console.log(
            chalk.white(`Updated Notion database ID: ${options.notionDb}`),
          );
        }
        ...
        if (options.organizeBy) {
          ...
          console.log(
            chalk.white(
              `Set Notion organization field to: ${options.organizeBy}`,
            ),
          );
        }
        ...
        if (options.groupBy) {
          ...
          console.log(
            chalk.white(`Set Notion grouping to: ${options.groupBy}`),
          );
        }
      ...
      if (
        ...
        if (options.slackChannel) {
          ...
          console.log(
            chalk.white(`Updated Slack channel: ${options.slackChannel}`),
          );
        }
        ...
        if (options.minSeverity) {
          ...
          console.log(
            chalk.white(
              `Set minimum severity level for Slack: ${options.minSeverity}`,
            ),
          );
        }
        ...
        if (options.format) {
          ...
          console.log(
            chalk.white(`Set Slack message format to: ${options.format}`),
          );
        }
      ...
      if (options.push) {
        ...
        const outputDir = path.join(
          workspacePath,
          parentOptions.output || "persrm-output",
        );
        ...
        try {
          ...
          console.error(
            chalk.red(
              `Error looking for analysis results: ${error instanceof Error ? error.message : "Unknown error"}`,
            ),
          );
          return;
        }
        ...
        for (const resultPath of resultFiles) {
          try {
            ...
            if (!results.summary) {
              console.log(
                chalk.yellow(
                  `No summary in ${path.basename(resultPath)}, skipping.`,
                ),
              );
              continue;
            }
            ...
            if (options.failures) {
              const hasCriticalIssues = results.summary.issues.some(
                (issue) =>
              ...
              if (!hasCriticalIssues) {
                if (!options.summary) {
                  console.log(
                    chalk.gray(
                      `Skipping ${path.basename(resultPath)}: No critical/error issues.`,
                    ),
                  );
                }
                continue;
            ...
            if (options.push === "analysis") {
              ...
              if (isNotionConfigured()) {
                const { pushAnalysisSummary } = await import(
                  "../lib/integrations/notion-sync"
                );
                const notionResult = await pushAnalysisSummary(
                  results.summary,
                  metadata,
                );
                ...
                if (notionResult) {
                  ...
                  if (options.copyLinks) {
                    ...
                    notionLinks.push({
                      component:
                        results.summary.componentName ||
                        results.summary.appName ||
                        "Unknown",
                      url: notionPageUrl,
                    });
                  }
              ...
              if (isSlackConfigured()) {
                const { sendAnalysisToSlack } = await import(
                  "../lib/integrations/slack-sync"
                );
                const slackResult = await sendAnalysisToSlack(
                  results.summary,
                  metadata,
                );
                ...
                if (slackResult) {
                  ...
                  if (options.copyLinks) {
                    const { getSlackConfig } = await import(
                      "../lib/integrations/slack-sync"
                    );
                    ...
                    if (
                      slackConfig &&
                      slackConfig.channel &&
                      slackConfig.channel.startsWith("#")
                    ) {
                      slackLinks.push({
                        component:
                          results.summary.componentName ||
              ...
              const { logBenchmark } = await import(
                "../lib/analytics/benchmark"
              );
              const benchmarkEntry = await logBenchmark(
                results.summary,
                baseMetadata,
              );

              // Get previous benchmark for comparison if available
              const { getBenchmarkHistory } = await import(
                "../lib/analytics/benchmark"
              );
              ...
              // Push to Notion if configured
              if (isNotionConfigured()) {
                const { pushBenchmarkData } = await import(
                  "../lib/integrations/notion-sync"
                );
                ...
                if (notionResult) {
                  if (options.summary) {
                    ...
                    console.log(
                      chalk.green("Successfully pushed benchmark to Notion"),
                    );
                  }
                  ...
                  if (options.summary) {
                    ...
                    console.log(
                      chalk.red("Failed to push benchmark to Notion"),
                    );
                  }
                }
              }
              ...
              // Push to Slack if configured
              if (isSlackConfigured()) {
                const { sendBenchmarkToSlack } = await import(
                  "../lib/integrations/slack-sync"
                );
                ...
                if (slackResult) {
                  if (options.summary) {
                    ...
                    console.log(
                      chalk.green("Successfully sent benchmark to Slack"),
                    );
                  }
                } else {
                  if (options.summary) {
              ...
              const regressionThreshold = parseFloat(
                options.regressionThreshold || "-5",
              );
              if (
                previousEntry &&
                benchmarkEntry.delta &&
                benchmarkEntry.delta.overallScore < regressionThreshold
              ) {
                if (!options.summary) {
                  console.log(
                    chalk.yellow(
                      `Regression detected: ${benchmarkEntry.delta.overallScore.toFixed(1)} points!`,
                    ),
                  );
                }
                ...
                if (isSlackConfigured()) {
                  const { sendRegressionAlertToSlack } = await import(
                    "../lib/integrations/slack-sync"
                  );

                  // Create affected phases array
                  const affectedPhases = Object.entries(
                    benchmarkEntry.delta.phaseDeltas,
                  )
                  ...
                  await sendRegressionAlertToSlack({
                    ...
                    component:
                      results.summary.componentName || "Unknown component",
                    project: results.summary.appName,
                  ...
                  if (slackConfig && slackConfig.aiInsights) {
                    const { sendAIInsightsToSlack } = await import(
                      "../lib/integrations/slack-sync"
                    );
              ...
              // Find regressions
              const { findRegressions } = await import(
                "../lib/analytics/benchmark"
              );
              const regressions = findRegressions(5, {
                // Find regressions with at least 5% drop
                component: results.summary.componentName,
                project: results.summary.appName,
              });
              ...
              console.log(
                chalk.yellow(
                  `Found ${regressions.length} regressions to report.`,
                ),
              );
              ...
              if (isSlackConfigured()) {
                ...
                for (const regression of regressions) {
                  ...
                  if (options.summary) {
                    process.stdout.write(
                      success ? chalk.green("R") : chalk.red("R"),
                    );
                    process.stdout.write(" ");
                  }
                }
                ...
                if (!options.summary) {
                  console.log(
                    chalk.green(
                      `Successfully sent ${successCount}/${regressions.length} regression alerts to Slack`,
                    ),
                  );
                } else {
              ...
              if (isSlackConfigured()) {
                ...
                if (success) {
                  if (options.summary) {
                    ...
                    console.log(
                      chalk.green("Successfully sent AI insights to Slack"),
                    );
                  }
                } else {
                  if (options.summary) {
                    ...
                    console.log(
                      chalk.red("Failed to send AI insights to Slack"),
                    );
                  }
                }
              ...
              console.log(
                chalk.white(
                  "Pushing learning data to configured integrations...",
                ),
              );

              // Get learning data
              const { getAllComponentData } = await import(
                "../lib/feedback/learner"
              );
              ...
              if (isNotionConfigured()) {
                const { pushLearningData } = await import(
                  "../lib/integrations/notion-sync"
                );
                ...
                for (const componentData of learningData) {
                  ...
                  if (options.summary) {
                    process.stdout.write(
                      notionResult ? chalk.green("N") : chalk.red("N"),
                    );
                    process.stdout.write(" ");
                  }
                }
                ...
                if (!options.summary) {
                  console.log(
                    chalk.green(
                      `Successfully pushed ${successCount}/${learningData.length} components to Notion`,
                    ),
                  );
                } else {
                  console.log(""); // Newline after summary output
              ...
              if (isSlackConfigured()) {
                const { sendLearningToSlack } = await import(
                  "../lib/integrations/slack-sync"
                );

                // Only send data for components with high-priority suggestions
                const componentsWithHighPriority = learningData.filter((data) =>
                  data.suggestions.some((s) => s.priority === "high"),
                );
                ...
                for (const componentData of componentsWithHighPriority) {
                  ...
                  if (options.summary) {
                    process.stdout.write(
                      slackResult ? chalk.green("S") : chalk.red("S"),
                    );
                    process.stdout.write(" ");
                  }
                }
                ...
                if (!options.summary) {
                  console.log(
                    chalk.green(
                      `Successfully sent ${successCount}/${componentsWithHighPriority.length} components to Slack`,
                    ),
                  );
                } else {
                  console.log(""); // Newline after summary output
            ...
            console.error(
              chalk.red(
                `Error processing ${path.basename(resultPath)}: ${error instanceof Error ? error.message : "Unknown error"}`,
              ),
            );
          }
        }
        ...
        if (
          options.copyLinks &&
          (notionLinks.length > 0 || slackLinks.length > 0)
        ) {
          console.log("");
          console.log(chalk.blue("Generated Links:"));
          ...
          if (notionLinks.length > 0) {
            ...
            notionLinks.forEach((link) => {
              console.log(
                chalk.white(`${link.component} (${link.type || "analysis"}):`),
              );
              console.log(chalk.blue.underline(link.url));
            });
          }
            ...
            slackLinks.forEach((link) => {
              ...
              console.log(
                chalk.blue.underline(
                  `slack://channel?team=TEAMID&id=${link.channel.substring(1)}`,
                ),
              );
              console.log(
                chalk.gray(`(Copy and replace TEAMID with your Slack team ID)`),
              );
            });
          }
        }
      ...
      if (options.test) {
        ...
        if (isNotionConfigured()) {
          try {
            ...
            if (notionResult) {
              console.log(
                chalk.green("✓ Notion integration is working correctly"),
              );

              // Display database URL for easy access
              const { getNotionConfig } = await import(
                "../lib/integrations/notion-sync"
              );
            ...
            console.log(
              chalk.red(
                `✗ Notion integration error: ${error instanceof Error ? error.message : "Unknown error"}`,
              ),
            );
          }
        } else {
        ...
        if (isSlackConfigured()) {
          try {
            const { testSlackConnection } = await import(
              "../lib/integrations/slack-sync"
            );
            ...
            if (slackResult) {
              console.log(
                chalk.green("✓ Slack integration is working correctly"),
              );
            ...
            console.log(
              chalk.red(
                `✗ Slack integration error: ${error instanceof Error ? error.message : "Unknown error"}`,
              ),
            );
          }
        } else {
      ...
      if (
        !Object.keys(options).some(
          (key) => options[key] !== undefined && key !== "_name",
        )
      ) {
        // Show Notion status
        ...
        if (isNotionConfigured()) {
          ...
          if (notionConfig && notionConfig.databaseId) {
            ...
            if (notionConfig.organizeBy) {
              console.log(
                chalk.blue(`  Organized by: ${notionConfig.organizeBy}`),
              );
            }
        ...
        if (isSlackConfigured()) {
          ...
          if (slackConfig) {
            ...
            if (slackConfig.minSeverity) {
              console.log(
                chalk.blue(`  Min severity: ${slackConfig.minSeverity}`),
              );
            }

            if (slackConfig.messageFormat) {
              console.log(
                chalk.blue(`  Message format: ${slackConfig.messageFormat}`),
              );
            }

            console.log(
              chalk.blue(
                `  AI insights: ${slackConfig.aiInsights ? "Enabled" : "Disabled"}`,
              ),
            );
          }
        } else {
        ...
        console.log(
          chalk.gray(
            "    persrm sync --notion-token YOUR_TOKEN --notion-db DATABASE_ID --enable-notion",
          ),
        );
        console.log(chalk.gray("  To configure Slack:"));
        console.log(
          chalk.gray(
            "    persrm sync --slack-webhook YOUR_WEBHOOK_URL --enable-slack",
          ),
        );
        console.log("");
        ...
        console.log(
          chalk.gray("    persrm sync --organize-by phase --group-by project"),
        );
      ...
      console.error(
        chalk.red(
          `Sync command failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        ),
      );
      process.exit(1);
    }
