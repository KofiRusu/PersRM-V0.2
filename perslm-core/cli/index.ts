...
program
  .name("persrm")
  .description(
    "Personalized UX React Monitor for component analysis and enhancement",
  )
  .version("1.0.0");
...
program
  ...
  .action((options) => {
    ...
    try {
      fs.mkdirSync(path.resolve(process.cwd(), config.outputDir), {
        recursive: true,
      });
    } catch (error) {
    ...
    const watchPaths = options.paths
      .split(",")
