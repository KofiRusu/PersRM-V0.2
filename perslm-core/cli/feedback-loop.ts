  ...
  .action(async () => {
    ...
    try {
      ...
      if (stats.failureCount > stats.totalEntries * 0.2) {
        console.log(
          chalk.yellow(
            "⚠️  High failure rate detected. Consider retraining the model.",
          ),
        );
      }
      ...
      if (stats.totalEntries > 50) {
        console.log(
          chalk.green(
            "✓ Enough data for LoRA fine-tuning. Run `pnpm retrain-model` to improve models.",
          ),
        );
      } else {
        console.log(
          chalk.yellow(
            "⚠️  Not enough data for LoRA fine-tuning yet. Collect more feedback.",
          ),
        );
      }
      ...
      if (componentsToImprove.length > 0) {
        console.log(
          chalk.green(
            `✓ Components with enough data for targeted improvement: ${componentsToImprove.join(", ")}`,
          ),
        );
      }
    } catch (error) {
      spinner.fail(
        chalk.red(
          `Error loading summary: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
      console.error(error);
      process.exit(1);
...
program
  ...
  .option(
    "-r, --result <type>",
    "Filter by result type (success, failure, improved)",
  )
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (options.result) {
        ...
        entries = await retentionService.getEntriesByComponent(
          options.component,
        );
      } else {
        // Combine all results
        const successEntries =
          await retentionService.getEntriesByResult("success");
        const failureEntries =
          await retentionService.getEntriesByResult("failure");
        const improvedEntries =
          await retentionService.getEntriesByResult("improved");
      ...
      if (entries.length === 0) {
        console.log(
          chalk.yellow("No entries found with the specified criteria."),
        );
        return;
      }
      ...
      if (viewDetailed) {
        const { entryIndex } = await inquirer.prompt([
          {
            ...
            validate: (value) => {
              ...
              return !isNaN(parsed) && parsed >= 0 && parsed < entries.length
                ? true
        ...
        console.log(
          chalk.cyan("Result:"),
          getColoredResult(selectedEntry.result),
        );
        console.log(
          chalk.cyan("Input:"),
        ...
        console.log(
          chalk.cyan("Error Type:"),
          selectedEntry.errorType || "N/A",
        );
        console.log(
          chalk.cyan("Remediation:"),
          selectedEntry.remediation || "N/A",
        );
        console.log(
          chalk.cyan("Metadata:"),
  ...
  .action(async (options) => {
    ...
    try {
        ...
        const { customComponent } = await inquirer.prompt([
          {
            ...
            validate: (value) =>
              value.trim() !== "" ? true : "Component name cannot be empty",
          },
        ]);
      ...
      const { resultType } = await inquirer.prompt([
        {
          ...
          choices: [
            {
              name: "Success - The output meets requirements",
              value: "success",
            },
            {
              name: "Improved - The output is usable but needs adjustments",
              value: "improved",
            },
            {
              name: "Failure - The output does not meet requirements",
              value: "failure",
            },
          ],
        },
      ...
      try {
        ...
        console.error(
          chalk.red("Error parsing JSON data. Please ensure it is valid JSON."),
        );
        process.exit(1);
      }
      ...
      const analysisPrompt = {
        ...
        generatedCode:
          typeof parsedOutput.code === "string"
            ? parsedOutput.code
      ...
      if (confirmStore) {
        ...
        if (resultType === "failure") {
          ...
          console.log(
            "1. Run `pnpm retrain-model -f failure` to improve the model based on failure cases",
          );
          console.log("2. Review action items and implement improvements");
          console.log(
            "3. Add a remediation plan with `pnpm feedback-loop remediate -t " +
              options.taskId +
              "`",
          );
        }
...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red(
          `Error storing remediation: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
      console.error(error);
      process.exit(1);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const spinner = ora(
        `Removing entries older than ${days} days...`,
      ).start();
      ...
      spinner.succeed(
        `Successfully removed ${count} entries older than ${days} days`,
      );
    } catch (error) {
      console.error(
        chalk.red(
          `Error cleaning up entries: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
      console.error(error);
      process.exit(1);
