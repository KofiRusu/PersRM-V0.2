...
program
  .option(
    "-f, --filter <type>",
    "Filter entries by result type (success, failure, improved, all)",
    "all",
  )
  .option("-c, --component <name>", "Filter entries by component name")
  .option(
    "-m, --min-entries <number>",
  ...
  .option(
    "--no-synthetic",
    "Do not generate synthetic examples to supplement training data",
  )
  .action(async (options) => {
    ...
    try {
      ...
      if (
        ...
        spinner.fail(
          chalk.red(
            `Invalid filter type: ${filter}. Must be one of: success, failure, improved, all`,
          ),
        );
        process.exit(1);
      }
      ...
      if (result.success) {
        if (result.promptEngineered) {
          spinner.succeed(
            chalk.green(
              `Successfully generated engineered prompts using ${result.entriesUsed} entries`,
            ),
          );
          console.log(
            chalk.cyan("Prompts saved to: data/engineered-prompts.json"),
          );
        } else {
          spinner.succeed(
            chalk.green(
              `Successfully fine-tuned model "${result.model}" using ${result.entriesUsed} entries`,
            ),
          );
          console.log(
            chalk.cyan(`Training completed in ${result.trainingTime} seconds`),
          );
