...
print(f"{BLUE}======================================{RESET}")
print(f"{BLUE}PersRM Task Monitor Runner{RESET}")
print(f"{BLUE}======================================{RESET}")
print()
...
# 2. Set up environment variables for the task monitor
os.environ["PORT"] = "3015"  # Use port 3015 specifically

# 3. Start the task monitor
print(f"{YELLOW}Starting Task Monitor on port 3015...{RESET}")
try:
    # Find the correct npm script
    with open('package.json', 'r') as f:
        package_content = f.read()
    
    # Determine appropriate command based on package.json
    if '"task-monitor"' in package_content:
        run_command = ["npm", "run", "task-monitor"]
    else:
        run_command = ["npm", "run", "dev"]  # fallback
    ...
    print(f"{YELLOW}Server starting on port 3015... waiting{RESET}")
    for _ in range(15):  # Wait up to 15 seconds
        print(".", end="", flush=True)
        time.sleep(1)
    print("\n")
    
    # 4. Open the task page in browser
    url = "http://localhost:3015/tasks"
    print(f"{GREEN}Opening task list in browser: {url}{RESET}")
    webbrowser.open(url)
