...
print(f"{BLUE}======================================{RESET}")
print(f"{BLUE}PersRM Analyzer Runner{RESET}")
print(f"{BLUE}======================================{RESET}")
print()
...
# 2. Set up environment variables for the analyzer
os.environ["PORT"] = "3016"  # Use port 3016 specifically
os.environ["NEXT_PUBLIC_APP_MODE"] = "analyzer"  # Set the app mode if needed

# 3. Start the analyzer app
print(f"{YELLOW}Starting Analyzer on port 3016...{RESET}")
try:
    ...
    print(f"{YELLOW}Server starting on port 3016... waiting{RESET}")
    for _ in range(15):  # Wait up to 15 seconds
        print(".", end="", flush=True)
        time.sleep(1)
    print("\n")
    
    # 4. Open the analyzer page in browser
    url = "http://localhost:3016/analyzer"
    print(f"{GREEN}Opening analyzer in browser: {url}{RESET}")
    webbrowser.open(url)
